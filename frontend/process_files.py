...
def upload_files_to_blob(files, dataset_name):
    # Connect to the Blob storage account using connection string
...
def fetch_configuration():
    ...
    try:
        ...
        st.warning("No Process found, create a new Process to get started.")
...
def process_files_tab():
    ...
    with col1:
        ...
        selected_dataset = st.selectbox("Select Process:", dataset_options)
    ...
    with col2:
        ...
        with st.expander("Add New Process"):
            ...
            new_dataset_name = st.text_input("New Process Name:")
            model_prompt = st.text_area("Model Prompt for new Process", "Extract all data.")
            example_schema = st.text_area("Example Schema for new Process", "{}")
